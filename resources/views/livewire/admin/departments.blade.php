            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6 sm:px-20 bg-white border-b border-gray-200">
                    ...
                    <div class="mt-6 text-gray-500">
                        Here you can manage all departments in the system.
                    </div>
                </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead>
                            <tr>
                                ...
                                <th wire:click="sortBy('faculty_id')"
                                    class="px-6 py-3 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider cursor-pointer">
                                    Faculty
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach ($departments as $department)
                                <tr>
                                    <td class="px-6 py-4 whitespace-no-wrap">{{ $department->name }}</td>
                                    <td class="px-6 py-4 whitespace-no-wrap">{{ $department->faculty->name }}</td>
                                    <td class="px-6 py-4 whitespace-no-wrap text-right text-sm leading-5 font-medium">
                                        <button wire:click="editDepartment({{ $department->id }})"
                                            class="text-indigo-600 hover:text-indigo-900">Edit</button>
                                        <button wire:click="deleteDepartmentConfirm({{ $department->id }})"
        ...
        <div class="fixed inset-0 flex items-center justify-center z-50">
            ...
            <div class="modal-container bg-white w-11/12 md:max-w-md mx-auto rounded shadow-lg z-50 overflow-y-auto">
                <div
                    ...
                    <h3 class="text-2xl font-bold">{{ $editMode ? 'Edit Department' : 'Create Department' }}</h3>
                ...
                <form wire:submit.prevent="{{ $editMode ? 'updateDepartment' : 'storeDepartment' }}" class="p-6">
                    <div class="mb-4">
                    ...
                    <div class="mb-4">
                        ...
                        <select
                            class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                            id="faculty_id" wire:model="faculty_id">
